% Decoding function for 'g' type strings
function [x, y, z, t1, t2] = decodeG(encoded_strings)
    ...
    if numel(parts) ~= 3
        error('Invalid encoded string format. Expected format "g:x,y,z,t1,t2"');
    end
        
    % Extract the character, coordinates and timings
    char = parts{1};
    coords_part = parts{2};
    timings_part = parts{3};
    ...
    if isnan(x) || isnan(y) || isnan(z) || isnan(t1) || isnan(t2)
